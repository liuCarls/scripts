...
ngApp.controller('myCtrl', ['$scope','$compile','$ocLazyLoad','$timeout','tabTools','menuFormat'
    ...
    $scope.menus = [{name:'Menu1', moduleName:'teModule',template:'mloadT/test.html', type:'nav'},
        {name:'Menu2', moduleName:'teModule',template:'mloadT/test.html', type:'nav'}];
    $scope.menu = $scope.menus[1];
    ...
    $scope.addTab = function($event, urlObj){
        ...
        if(needNew){
            ...
            tabScope.type = paramObj.type;
            tabScope.htmlUrl = paramObj.template;
            tabScope.moduleName = paramObj.moduleName;
            tabScope.title = paramObj.name;
            tabScope.selected = true;
            this.selectTab(tabScope);

            //选项卡内容生成与异步加载
            var tpl = $compile( `<div ng-show="selected"><div id="teModule" oc-lazy-load="teModule">
        ...
        if( tabScope.type == 'nav' ){
            ...
            this.tabs.splice(0,1,tabScope); //菜单只保留一个工作区，并且不生产Tab
